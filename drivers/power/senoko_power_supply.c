			       ...
			       union power_supply_propval *val)
{
	...
	senoko_supply = power_supply_get_drvdata(psy);
...
static int senoko_power_supply_probe(struct platform_device *pdev)
{
	...
	power_supply_desc = &senoko_supply->supply_desc;

	power_supply_desc->name = "senoko-ac-supply";
	power_supply_desc->type = POWER_SUPPLY_TYPE_MAINS;
	power_supply_desc->properties = senoko_power_supply_props;
	power_supply_desc->num_properties = ARRAY_SIZE(senoko_power_supply_props);
	power_supply_desc->get_property = senoko_get_property;
	...
	psy_cfg.supplied_to = senoko_supply->supplicant_names;
	psy_cfg.num_supplicants = senoko_supply->num_supplicants;
	psy_cfg.of_node = pdev->dev.of_node;
	psy_cfg.drv_data = senoko_supply;

	senoko_supply->supply = power_supply_register(senoko_supply->dev,
						      power_supply_desc,
						      &psy_cfg);
	if (IS_ERR(senoko_supply->supply)) {
		ret = PTR_ERR(senoko_supply->supply);
